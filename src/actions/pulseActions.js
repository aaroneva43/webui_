...
import * as types from './pulseTypes';
...
export function loadEvents(api_uri) {
...
export function loadEvents2(api_uri) {
  ///console.log('url: ', api_uri);
    return dispatch => {
    ///dispatch(loadEventsRequest());
       return request
      ...
      .end((err, res) => {
        if (err) {
            ///console.log('errrr: ', err);
          dispatch(loadEventsFailure2(err));
        } else {
            ///console.log('res ::: ', res);
          dispatch(loadEventsSuccess2(res.body.data));
        }
      });
    }
}
...
//     console.log('Add event 2');
    ...
    return request
      ...
      .send(event)
      .set('Accept', 'application/json')
      ///.set('Authorization', 'Basic b25vczpyb2Nrcw==')
      .end((err, res) => {
        if (err) {
            console.log('err: ', err);
...
export function loadEventsRequest() {
    ///console.log('made it to loadeventrequest');
  return {
...
export function loadEventsSuccess(events) {
    console.log('loadevent success: ', events)
  return {
...
// export function loadEventsSuccess2(events2) {
//   return {
//     type: types.LOAD_EVENTS_SUCCESS,
//     events2
//   };
...
export function loadEventsFailure2(error) {
  return {
    type: types.LOAD_EVENTS_FAILURE,
    error2
  };
